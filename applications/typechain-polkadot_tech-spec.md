...
----------
# TODO
- The rest of the arguments types
- - Broaden the types of arguments & returns, when aplicable
- - Type of `value` prop in `{ gasLimit, value }`
- Return types for transactions - if possible
- - Always can query, get response and then tx
- Clarify `AccountId:string` type (at least with comments)
- Optimize parsing algorythms
- Make sure it works on Windows
- - Especially `bin` script
- Document this package
- Conceal parser code in published package
	// Might have to build package into JS after all
- Support for V1 & V2 ABIs
- Investigate possibility to fetch ABIs by contracts' addresses only
	// This way user only requires to know addresses of the contracts & doesn't need to provide ABIs
